...
- Metadata extraction from PDF properties and extracted PDF text, including file type, modification date, title, subject, summary, keywords, publication place/date, and repeated headers or footers when available.
...
Images, Word documents, and other file formats are not supported in this
version.
...
- `rag_dataprep_agent/evaluation/` - ground-truth loading, deterministic metrics, reporting, and optional OpenAI summary judging.
...
GitHub Actions runs `uv sync --locked` and the complete pytest suite for every
pull request targeting `main` and every push to `main`. The `main` branch is
protected so a pull request cannot be merged unless the `pytest` status check
passes. The workflow is defined in `.github/workflows/tests.yml`.

## Evaluation

The project includes ground-truth JSON for 14 selected evaluation documents.
These expected values were manually authored and reviewed by the project owner;
they were not generated by an LLM. The fields, matching rules, metrics, and
...
- Support additional file formats such as Word documents, presentations, spreadsheets, and images.
- Process documents written in languages other than English.
- Improve keyword extraction so it produces meaningful topic phrases.
- Add pseudonymisation of sensitive information.
- Add retrieval and question answering over prepared documents.
- Expose retrieval and metadata inspection as LLM-callable tools.
